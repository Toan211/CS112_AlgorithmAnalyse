...
def knapsackReduce(items, W):
	...
	for item in items:
		for w in range(W, item[2] - 1, -1):
			value = best_value[w - item[2]][0] + item[1]

			if value > best_value[w][0]:
				best_value[w] = (value, [item[0]] + best_value[w - item[2]][1])
...
def complementaryknapsack(items):
	...
	return TotalPop - value[0], value[1]



value, listOfBangBiBo = complementaryknapsack(listOfBang)

for bang in listOfBang:
	if bang[0] not in listOfBangBiBo:
		print(bang[0])